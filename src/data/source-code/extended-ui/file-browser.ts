...
export const fileBrowserSource: ComponentSourceData = {
  component: 'File Browser',
  slug: 'file-browser',
  category: 'Extended UI',
  categorySlug: 'extended-ui',
  description: 'File Browser component for the Extended UI library with full customization support.',
  sources: {
    react: { language: 'tsx', filename: 'FileBrowser.tsx', code: `import React, { useState } from 'react';
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
      ...
      {active && (
        <div className="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
          <p className="text-xs text-gray-600 dark:text-gray-400">Interactive file browser content</p>
        </div>
      )}
    </div>
  );
}` },
    nextjs: { language: 'tsx', filename: 'file-browser/page.tsx', code: `'use client';
import FileBrowser from '@/components/mtverse/extended-ui/FileBrowser';
export default function Page() {
  return <FileBrowser />;
}` },
    vue: { language: 'vue', filename: 'FileBrowser.vue', code: `<template>
  <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
    ...
    <div v-if="active" class="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
      <p class="text-xs text-gray-600 dark:text-gray-400">Interactive file browser content</p>
    </div>
  </div>
</template>
...
const active = ref(false);
    ...
    angular: { language: 'ts', filename: 'file-browser.component.ts', code: `import { Component } from '@angular/core';
@Component({
...
export class FileBrowserComponent {
  ...
  toggle() { this.active = !this.active; }
    ...
    html: { language: 'html', filename: 'file-browser.html', code: `<!-- HTML + Alpine.js -->
<div x-data="{ active: false }" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
  ...
  <div x-show="active" x-transition class="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
    ...
    laravel: { language: 'blade', filename: 'file-browser.blade.php', code: `<!-- Laravel Blade + Alpine.js -->
<div x-data="{ active: false }" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
  <div class="flex items-center justify-between">
    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">File Browser</span>
    <button @click="active = !active"
