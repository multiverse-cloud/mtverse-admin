...
export const habitTrackerSource: ComponentSourceData = {
  component: 'Habit Tracker',
  slug: 'habit-tracker',
  category: 'Extended UI',
  categorySlug: 'extended-ui',
  description: 'Habit Tracker component for the Extended UI library with full customization support.',
  sources: {
    react: { language: 'tsx', filename: 'HabitTracker.tsx', code: `import React, { useState } from 'react';
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
      ...
      {active && (
        <div className="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
          <p className="text-xs text-gray-600 dark:text-gray-400">Interactive habit tracker content</p>
        </div>
      )}
    </div>
  );
}` },
    nextjs: { language: 'tsx', filename: 'habit-tracker/page.tsx', code: `'use client';
import HabitTracker from '@/components/mtverse/extended-ui/HabitTracker';
export default function Page() {
  return <HabitTracker />;
}` },
    vue: { language: 'vue', filename: 'HabitTracker.vue', code: `<template>
  <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
    <div class="flex items-center justify-between">
      <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Habit Tracker</span>
    ...
    <div v-if="active" class="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
      <p class="text-xs text-gray-600 dark:text-gray-400">Interactive habit tracker content</p>
    </div>
  </div>
</template>
...
const active = ref(false);
    ...
    angular: { language: 'ts', filename: 'habit-tracker.component.ts', code: `import { Component } from '@angular/core';
@Component({
...
export class HabitTrackerComponent {
  ...
  toggle() { this.active = !this.active; }
    ...
    html: { language: 'html', filename: 'habit-tracker.html', code: `<!-- HTML + Alpine.js -->
<div x-data="{ active: false }" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
  ...
  <div x-show="active" x-transition class="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
    ...
    laravel: { language: 'blade', filename: 'habit-tracker.blade.php', code: `<!-- Laravel Blade + Alpine.js -->
<div x-data="{ active: false }" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
  <div class="flex items-center justify-between">
    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Habit Tracker</span>
    <button @click="active = !active"
