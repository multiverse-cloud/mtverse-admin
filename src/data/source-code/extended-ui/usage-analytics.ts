...
export const usageAnalyticsSource: ComponentSourceData = {
  component: 'Usage Analytics',
  slug: 'usage-analytics',
  category: 'Extended UI',
  categorySlug: 'extended-ui',
  description: 'Usage Analytics component for the Extended UI library with full customization support.',
  sources: {
    react: { language: 'tsx', filename: 'UsageAnalytics.tsx', code: `import React, { useState } from 'react';
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
      ...
      {active && (
        <div className="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
          <p className="text-xs text-gray-600 dark:text-gray-400">Interactive usage analytics content</p>
        </div>
      )}
    </div>
  );
}` },
    nextjs: { language: 'tsx', filename: 'usage-analytics/page.tsx', code: `'use client';
import UsageAnalytics from '@/components/mtverse/extended-ui/UsageAnalytics';
export default function Page() {
  return <UsageAnalytics />;
}` },
    vue: { language: 'vue', filename: 'UsageAnalytics.vue', code: `<template>
  <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
    <div class="flex items-center justify-between">
      <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Usage Analytics</span>
    ...
    <div v-if="active" class="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
      <p class="text-xs text-gray-600 dark:text-gray-400">Interactive usage analytics content</p>
    </div>
  </div>
</template>
...
const active = ref(false);
    ...
    angular: { language: 'ts', filename: 'usage-analytics.component.ts', code: `import { Component } from '@angular/core';
@Component({
...
export class UsageAnalyticsComponent {
  ...
  toggle() { this.active = !this.active; }
    ...
    html: { language: 'html', filename: 'usage-analytics.html', code: `<!-- HTML + Alpine.js -->
<div x-data="{ active: false }" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
  ...
  <div x-show="active" x-transition class="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
    ...
    laravel: { language: 'blade', filename: 'usage-analytics.blade.php', code: `<!-- Laravel Blade + Alpine.js -->
<div x-data="{ active: false }" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
  <div class="flex items-center justify-between">
    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Usage Analytics</span>
    <button @click="active = !active"
