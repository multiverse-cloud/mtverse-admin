...
export const yamlEditorSource: ComponentSourceData = {
  component: 'Yaml Editor',
  slug: 'yaml-editor',
  category: 'Extended UI',
  categorySlug: 'extended-ui',
  description: 'Yaml Editor component for the Extended UI library with full customization support.',
  sources: {
    react: { language: 'tsx', filename: 'YamlEditor.tsx', code: `import React, { useState } from 'react';
    ...
    <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
      ...
      {active && (
        <div className="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
          <p className="text-xs text-gray-600 dark:text-gray-400">Interactive yaml editor content</p>
        </div>
      )}
    </div>
  );
}` },
    nextjs: { language: 'tsx', filename: 'yaml-editor/page.tsx', code: `'use client';
import YamlEditor from '@/components/mtverse/extended-ui/YamlEditor';
export default function Page() {
  return <YamlEditor />;
}` },
    vue: { language: 'vue', filename: 'YamlEditor.vue', code: `<template>
  <div class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
    ...
    <div v-if="active" class="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
      <p class="text-xs text-gray-600 dark:text-gray-400">Interactive yaml editor content</p>
    </div>
  </div>
</template>
...
const active = ref(false);
    ...
    angular: { language: 'ts', filename: 'yaml-editor.component.ts', code: `import { Component } from '@angular/core';
@Component({
...
export class YamlEditorComponent {
  ...
  toggle() { this.active = !this.active; }
    ...
    html: { language: 'html', filename: 'yaml-editor.html', code: `<!-- HTML + Alpine.js -->
<div x-data="{ active: false }" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
  ...
  <div x-show="active" x-transition class="mt-3 rounded-md bg-gray-50 p-3 dark:bg-white/5">
    ...
    laravel: { language: 'blade', filename: 'yaml-editor.blade.php', code: `<!-- Laravel Blade + Alpine.js -->
<div x-data="{ active: false }" class="rounded-lg border border-gray-200 bg-white p-4 dark:border-white/10 dark:bg-gray-800">
  <div class="flex items-center justify-between">
    <span class="text-sm font-medium text-gray-700 dark:text-gray-300">Yaml Editor</span>
    <button @click="active = !active"
